...
/** 在线 Google / 离线 Leaflet（伪 BMap） */
type ScreenMapMode = 'google' | 'offline' | null
let screenMapMode: ScreenMapMode = null
...
function renderTrajectoryGoogle(): void {
  if (!googleMap) return
  if (currentPolylineGoogle) {
    currentPolylineGoogle.setMap(null)
    currentPolylineGoogle = null
  }
  const r = robots.find((item) => item.id === selectedId.value)
  if (!r || !r.track || r.track.length === 0) return
  if (!r.showOnMap) return
  const path = r.track.map((p) => ({ lat: p.lat, lng: p.lon }))
  currentPolylineGoogle = new google.maps.Polyline({
    path,
    strokeColor: '#FF0000',
    strokeWeight: 3,
    strokeOpacity: 0.8,
...
function renderMarkersGoogle(): void {
  if (!googleMap) return
  const map = googleMap
  const activeIds = new Set(robots.map((r) => r.id))
  for (const [id, marker] of markersGoogle.entries()) {
    if (!activeIds.has(id)) {
      marker.setMap(null)
      markersGoogle.delete(id)
    }
  }
  robots.forEach((r) => {
    if (!r.showOnMap) {
      const existing = markersGoogle.get(r.id)
      if (existing) {
        existing.setMap(null)
        markersGoogle.delete(r.id)
      }
      return
    }
    const isSelected = r.id === selectedId.value
    const zIndex = isSelected ? 999 : 100
    const icon = fishIconGoogle(isSelected)
    let marker = markersGoogle.get(r.id)
    if (marker) {
...
function renderTrajectory(): void {
  if (screenMapMode === 'google') {
    renderTrajectoryGoogle()
    return
  }
  const BMap = getBMap()
...
watch(selectedId, (newId) => {
  ...
  if (target && screenMapMode === 'google' && googleMap) {
    googleMap.setCenter({ lat: target.lat, lng: target.lng })
    googleMap.setZoom(14)
  } else if (target && mapInstance) {
    const BMap = getBMap()
    if (BMap) {
...
async function init(): Promise<void> {
  try {
    ...
    screenMapMode = useOffline ? 'offline' : 'google'

    if (useOffline) {
      await loadOfflineBMap()
    } else if (!GOOGLE_KEY.trim()) {
      console.error('VITE_GOOGLE_MAPS_API_KEY is not set; cannot load Google Maps')
      screenMapMode = null
      return
    } else {
      try {
        ...
        console.warn('First attempt to load Google Maps failed, retrying...', e)
        await new Promise((resolve) => setTimeout(resolve, 1000))
        try {
          await loadGoogleMaps(GOOGLE_KEY)
        } catch (retryErr) {
          console.error('Retry loading Google Maps failed:', retryErr)
        }
      }
    }
    ...
    if (screenMapMode === 'google') {
      ...
      const BMap = getBMap()
      if (!BMap) {
        console.error('Offline map API not available after load')
        return
      }
...
<template>
  <section class="screen">
    <div class="layout">
      <div class="map-panel">
        <div id="map-container" class="bmap"></div>
